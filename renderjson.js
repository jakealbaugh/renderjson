...
var module, window, define, renderjson = (function() {
  ...
  var prefixed = function(string) {
    string = string.replace(/ +/g, ' ');
    return string.split(' ').map(function(s) { return prefix + s }).join(' ');
  }
  ...
  var span = function(classname) {
    ...
    if (classname) s.className = prefixed(classname);
    return s;
  };
  var A = function A(txt, classname, callback) {
    var a = document.createElement("a");
    if (classname) a.className = prefixed(classname);
  ...
  renderjson.set_show_to_level = function(level) {
    var all = typeof level == "string" && level.toLowerCase() === "all";
    renderjson.options.show_to_level = all ? Number.MAX_VALUE : level;
    return renderjson;
  };
  renderjson.set_max_string_length = function(length) {
    var none = typeof length == "string" && length.toLowerCase() === "none"
    renderjson.options.max_string_length = none ? Number.MAX_VALUE : length;
    return renderjson;
  };
